...
class DerivedLogger : public LoggerBase<OpType, Vector>
{
...
public:
    ...
    void iteration_log(const Index& iteration, const Index& number_of_converged, const Index& subspace_size, const Vector& current_eigenvalues, const Vector& residues, const BoolArray& current_eig_converged) override
    {
        ...
        std::cout << "    Size of Krylov subspace         :   " << subspace_size << std::endl;
        std::cout << "    ------------------------------------------------------------------------              " << std::endl;
...
void run_test_sets(const MatType& A, int k, int m)
{
    ...
    OpType op(A);
    std::unique_ptr<LoggerBase<OpType, ComplexVector>> logger(new DerivedLogger<OpType, ComplexVector>());
