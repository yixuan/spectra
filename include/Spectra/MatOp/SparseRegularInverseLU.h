...
#ifndef SPECTRA_SPARSE_REGULAR_INVERSE_LU_H
#define SPECTRA_SPARSE_REGULAR_INVERSE_LU_H
...
///
/// This class is intended to be used with the KrylovSchurGEigsSolver generalized eigen solver
...
private:
    ...
    const Index m_n;
    Eigen::SparseLU<SparseMatrix, Eigen::COLAMDOrdering<int>> m_cg;
...
public:
    ...
    SparseRegularInverseLU(const Eigen::SparseMatrixBase<Derived>& mat) :
        m_mat(mat), m_n(mat.rows())
    {
        static_assert(
            static_cast<int>(Derived::PlainObject::IsRowMajor) == static_cast<int>(SparseMatrix::IsRowMajor),
            "SparseRegularInverseLU: the \"Flags\" template parameter does not match the input matrix (Eigen::ColMajor/Eigen::RowMajor)");

        if (mat.rows() != mat.cols())
            throw std::invalid_argument("SparseRegularInverseLU: matrix must be square");

        // Eigen::SparseLU
        //m_cg.isSymmetric(true);
    ...
    void solve(const Scalar* x_in, Scalar* y_out) const
    {
        ...
        if (m_info != CompInfo::Successful)
            throw std::runtime_error("SparseRegularInverseLU: CG solver does not converge");
    }
...
#endif  // SPECTRA_SPARSE_REGULAR_INVERSE_LU_H
